...
function arrayEquals(one: Array<any>, two: Array<any>): boolean {
...
function recordEquals(one: Record<string, any>, two: Record<string, any>): boolean {
	for (let key in one) {
		if (!(key in two)) {
			return false;
		}
...
(() => {
	...
	test(`It should decode map {} properly.`, async () => {
		...
		let expected = {} as Record<string, any>;
	...
	test(`It should decode map { key1: "one" } properly.`, async () => {
		...
		let expected = { key1: "one" } as Record<string, any>;
		console.assert(recordEquals(observed, expected));
	});
	...
	test(`It should decode map { key1: "one", key2: "two" } properly.`, async () => {
		...
		let expected = { key1: "one", key2: "two" } as Record<string, any>;
		console.assert(recordEquals(observed, expected));
	});
	...
	test(`It should decode map { key2: "two", key1: "one" } properly.`, async () => {
		...
		let expected = { key2: "two", key1: "one" } as Record<string, any>;
		console.assert(recordEquals(observed, expected));
	});
})();
...
(() => {
	...
	test(`It should decode ["one", 0n, false] as tuple<string, bigint> properly.`, async () => {
		...
		let expected = ["one", 0n, false] as any[];
		console.assert(arrayEquals(observed, expected));
	});
})();
...
(() => {
	...
	test(`It should encode { key1: "a", key2: "b" } as object<{ key1: string }> properly.`, async () => {
		let codec = codecs.Object.of({
			key1: codecs.String
		});
		let observed = codec.encode({
	...
	test(`It should decode { key1: "a", key2: "b" } as object<{ key1: string }> properly.`, async () => {
		let codec = codecs.Object.of({
			key1: codecs.String
		});
		let observed = codec.decode(Uint8Array.of(0x13, 0x08, 0x05, 0x04, 0x6B, 0x65, 0x79, 0x31, 0x02, 0x04, 0x61, 0x05, 0x04, 0x6B, 0x65, 0x79, 0x32, 0x02, 0x04, 0x62));
	...
	test(`It should encode { key1: "a" } as object<{ key1: string }, { key2: string }> properly.`, async () => {
		let codec = codecs.Object.of({
			key1: codecs.String
		}, {
			key2: codecs.String
	...
	test(`It should decode { key1: "a" } as object<{ key1: string }, { key2: string }> properly.`, async () => {
		let codec = codecs.Object.of({
			key1: codecs.String
		}, {
			key2: codecs.String
